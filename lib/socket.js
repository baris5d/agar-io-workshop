...
const { BLOCKS_COUNT, BLOCKS_SIZE } = require("../constants");
...
module.exports = (io, players, field, blobs) => {
  io.sockets.on("connection", (socket) => {
    ...
    socket.on("start", (data) => {
      ...
      socket.emit("start", {
        block: [posX, posY],
        players: players,
        blobs: getNeighbourBlobs(posX, posY),
      });

      console.log(
        "> [LOG] Player",
        `${socket.id}`.gray,
        "started playing at block",
        `${pos}`.magenta
      );
    ...
    socket.on("update", (data) => {
      ...
      if (
        ...
        socket.emit("update_blobs", getNeighbourBlobs(posX, posY));
      }
    ...
    socket.on("eatBlob", (i) => {
      ...
      const killedPlayers = Object.values(players)
        .filter((player) => player.id != socket.id)
        .filter((player) => calculate(me, player));

      if (killedPlayers.length > 0) {
        killedPlayers.filter((player) => {
          delete players[player.id];

          // Say that to sockets
          io.sockets.to(player.id).emit("gameOver", player.id);
  ...
  function calculate(me, player) {
    var dX = me.x - player.x;
    var dY = me.y - player.y;

    var distance = Math.sqrt(dX * dX + dY * dY);

    if (distance < me.r + player.r) {
      return me.r > player.r;
    }
  ...
  function getNeighbourBlobs(posX, posY) {
    let neighbourBlobs = field[posX][posY].blobs.concat(
      field[posX - 1] && field[posX - 1][posY - 1]
        ? field[posX - 1][posY - 1].blobs
        : [],
      ...
      field[posX - 1] && field[posX - 1][posY]
        ? field[posX - 1][posY].blobs
        : [],
      field[posX + 1] && field[posX + 1][posY]
        ? field[posX + 1][posY].blobs
        : [],
